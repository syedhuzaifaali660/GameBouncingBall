...
public class HelixController : MonoBehaviour {
    ...
    void Awake () {
        //Converts the angels in Vector3
        startRotation = transform.localEulerAngles;
        helixDistance = topTransform.localPosition.y - (goalTransform.localPosition.y + 0.1f); // distance from top and obtton level
        LoadStage(0);
    }
	...
	void Update () {
        ...
        if (Input.GetMouseButton(0))
        {
            if (EventSystem.current.IsPointerOverGameObject())
                return;
            // point where we touch the screen
            Vector2 curTapPos = Input.mousePosition;

            //checking last tap = vector2.zero meaning if last tap position was empty or zero
            if (lastTapPos == Vector2.zero)
                lastTapPos = curTapPos;
            //calculating the diffrence b/w lasttap and current tap it will follow along with our finger where we touched
            float delta = lastTapPos.x - curTapPos.x;
            lastTapPos = curTapPos;
            //rotate the helix 
            transform.Rotate(Vector3.up * delta);
        }
        //as soon as we release the mouse button
        ...
        for (int i = 0; i < stage.levels.Count; i++)
        {
            ...
            while (deathParts.Count < stage.levels[i].deathPartCount)
            {
                ...
                if (!deathParts.Contains(randomPart))
                {
                    ...
                    FindObjectOfType<DeathPart>().GetComponent<Renderer>().material.color = allStages[stageNumber].stageBallColor;
